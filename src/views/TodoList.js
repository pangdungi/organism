...
function rollbackRemoveTaskFromCustomSectionStorage(
  sectionId,
  taskId,
  snapshot,
) {
...
/** 예전 「우선순위」「날짜」집계 탭용 — 현재 FIXED_SECTIONS에 없음(빈 배열 유지 시 관련 분기 무해화) */
...
function todoListStorageSectionIdFromEl(el) {
  ...
  if (pid === TODO_ALL_TAB_SECTION_ID) {
    return (el.dataset.sectionId || "").trim();
  }
  if (
...
function scheduleSaveSectionTasksFromDOM(sectionsWrap) {
  ...
  _saveSectionTasksTimer = setTimeout(() => {
    _saveSectionTasksTimer = null;
    collectAndSaveSectionTasksFromDOM(sectionsWrap);
  }, 300);
}
...
/** 고정 섹션 카드/행을 DOM에서 읽어 세션 메모리만 갱신. 서버 호출 없음. */
function collectAndSaveSectionTasksFromDOM(sectionsWrap) {
  todoDebug("collectAndSaveSectionTasksFromDOM", { hasWrap: !!sectionsWrap });
  if (!sectionsWrap) return;
...
function getFixedSectionsWithAllTab() {
  return [{ id: TODO_ALL_TAB_SECTION_ID, label: "전체" }, ...FIXED_SECTIONS];
}
  ...
  modal.innerHTML = `
    ...
    <div class="time-task-setup-panel time-add-task-panel">
      <div class="time-task-setup-header">
        <h3 class="time-task-setup-title">${escapeConfirmHtml(title)}</h3>
...
function showMobileDateModal(options) {
  ...
  modal.innerHTML = `
    ...
    <div class="time-task-setup-panel time-add-task-panel">
      <div class="time-task-setup-header">
        <h3 class="time-task-setup-title">${title}</h3>
        <button type="button" class="time-task-setup-close" aria-label="닫기">&times;</button>
      </div>
...
function showTodoTaskModal(options) {
  ...
  const modal = document.createElement("div");
  modal.className = "time-task-setup-modal time-add-task-modal";
  modal.innerHTML = `
    <div class="time-task-setup-backdrop"></div>
    <div class="time-task-setup-panel time-add-task-panel">
      <div class="time-task-setup-header">
      ...
      <div class="time-task-log-footer todo-task-edit-footer todo-task-edit-footer--actions">
        ...
        <button type="button" class="time-add-task-submit todo-task-edit-footer-confirm">${mode === "add" ? "추가" : "저장"}</button>
      </div>
    </div>
  `;
...
function createTaskRow(taskData = {}, options = {}) {
  ...
  const sourceSectionIdForRow = String(taskData.sourceSectionId || "").trim();
...
function mirrorTodoCardElementFromPrimary(duplicateCard, primaryCard) {
  ...
  if (p.dataset.sourceSectionId) {
    d.dataset.sourceSectionId = p.dataset.sourceSectionId;
  } else {
    d.removeAttribute("data-source-section-id");
  }
  d.dataset.name = p.dataset.name || "";
  d.dataset.startDate = p.dataset.startDate || "";
  d.dataset.dueDate = p.dataset.dueDate || "";
  d.dataset.reminderDate = "";
  d.dataset.reminderTime = "";
...
function createTaskCard(taskData, options = {}) {
  ...
  if (
    String(sectionId || "").trim() === "overdue" &&
    String(sourceSectionId || "").trim()
  ) {
    card.dataset.sourceSectionId = String(sourceSectionId).trim();
  }
      ...
      onSave: (payload) => {
        ...
        if (newSectionId && newSectionId !== storageSectionId) {
          ...
          if (targetWrap) {
            ...
            if (String(newSectionId || "").trim() !== "overdue") {
              card.removeAttribute("data-source-section-id");
            }
...
/** 날짜·우선순위 집계 탭용: 저장소에서 할 일 목록을 메인 뷰 필터와 동일하게 고름(고정 섹션 작업만) */
...
function refreshTodoDateTabSectionDom(sectionsWrap) {
  ...
  panel
    .querySelectorAll(".todo-priority-quadrant[data-date-bucket]")
    .forEach((cell) => {
  ...
  function updateCount() {
    ...
    if (countSpan) {
      countSpan.textContent = String(
        panel.querySelectorAll(".todo-card").length,
      );
    }
  }
...
/** 「전체」탭: 카테고리 패널 DOM·메모리와 동일 목록이 되도록 다시 채움(섹션 이동·추가 후 동기화) */
function refreshTodoAllTabSectionDom(sectionsWrap) {
  const panel = sectionsWrap?.querySelector(
    `.todo-section[data-section="${TODO_ALL_TAB_SECTION_ID}"]`,
  );
  const cardsWrap = panel?.querySelector(".todo-cards-wrap");
  if (!cardsWrap || !sectionsWrap) return;
  flushSaveSectionTasksFromDOM(sectionsWrap);
  const enableDragToCalendar = sectionsWrap.dataset.lpDragCal === "1";
  const enableDragToEisenhower = sectionsWrap.dataset.lpDragEisen === "1";
  ...
  function updateCount() {
    const el = panel.querySelector(".todo-section-count");
    if (el)
      el.textContent = String(cardsWrap.querySelectorAll(".todo-card").length);
  }

  cardsWrap.replaceChildren();
  for (const sid of FIXED_SECTION_IDS_FOR_STORAGE) {
    const arr = loadSectionTasks(sid);
    for (const raw of arr) {
      const t = { ...raw, sectionId: sid };
      const taskId = t.taskId || getTaskId(t);
      t.taskId = taskId;
      const card = createTaskCard(t, {
        updateCount,
  ...
  sections.forEach((section) => {
    const sectionTasks =
      ...
      section.id === TODO_DATE_TAB_SECTION_ID
        ? tasksData
        : section.id === TODO_ALL_TAB_SECTION_ID
          ? FIXED_SECTION_IDS_FOR_STORAGE.flatMap((sid) =>
              tasksData.filter((t) => t.sectionId === sid),
            )
...
export function render(options = {}) {
  ...
  const reusedSettings =
    ...
    (reuseSettingsButtonEl.hasAttribute(TODO_LIST_SETTINGS_BTN_ATTR) ||
  ...
  const settingsBtn = reusedSettings
    ...
    : (() => {
        ...
        b.setAttribute(TODO_LIST_SETTINGS_BTN_ATTR, "");
        b.title = "할 일 환경 설정";
  ...
  /* 할일/일정: 「전체」+ 고정 4탭, 리스트 추가 비노출 */
  getFixedSectionsWithAllTab().forEach((section) => {
  ...
  function taskItemPassesSectionListFilter(domEl) {
    ...
    if (
      el.classList.contains("section-task-filter-todo-only") &&
      t === "schedule"
    ) {
      return false;
    }
  ...
  if (hasExplicitInitialTab) {
    ...
    try {
      let raw = sessionStorage.getItem(SESSION_TODO_FIXED_TAB_INDEX);
      if (raw == null) {
        raw = sessionStorage.getItem(SESSION_TODO_FIXED_TAB_INDEX_LEGACY);
        if (raw != null) {
          const legacy = parseInt(raw, 10);
          if (
            !Number.isNaN(legacy) &&
            legacy >= 0 &&
            legacy <= FIXED_SECTIONS.length - 1
          ) {
            initialActiveTabIndex = legacy + 1;
          }
        }
      } else {
  ...
  function reconnectTodoSectionsMutationObserver() {
    try {
      observer.disconnect();
    } catch (_) {}
  ...
  if (categoryToolbarRightActions) {
    ...
    searchFilterObserver.observe(sectionsWrap, {
      childList: true,
      subtree: true,
    });
  }
  ...
  function remountTodoSectionsAfterCalendarPull() {
    ...
    sectionResults.forEach((r, i) => {
      r.wrap.classList.toggle("is-active", i === activeSectionIndex);
    });
    tabButtons.forEach((b, i) => {
      b.classList.toggle("active", i === activeSectionIndex);
    });
    syncTodoListSegmentThumb();
    reconnectTodoSectionsMutationObserver();
    updateTabLabels();
