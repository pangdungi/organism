...
function setupDeadlineQuickButtons(modal) {
  ...
  modal.querySelectorAll(".dream-kpi-deadline-quick-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const startVal = startInput?.value?.trim();
      const baseDate = deadlineVal
        ? new Date(deadlineVal + "T12:00:00")
        : startVal
...
export function render() {
  ...
  function showKpiLogModal(kpi, editLog) {
    ...
    modal
      ...
      .addEventListener("click", close);
    modal
      .querySelector(".dream-kpi-log-form")
      .addEventListener("submit", (e) => {
        e.preventDefault();
        const form = e.target;
        const dateVal = form.date.value;
        const dateStr = dateVal
          ? `${dateVal.split("-")[0]}. ${dateVal.split("-")[1]}. ${dateVal.split("-")[2]}.`
          : toDateStr(new Date());
        const data = loadHappinessMap();
        if (isEdit) {
  ...
  function renderKpiList() {
    ...
    if (completedKpis.length > 0 && kpiFilter !== "completed") {
      ...
      completedSection.className =
        "dream-kpi-completed-section" +
        (completedSectionCollapsed ? " is-collapsed" : "");
      ...
      `;
      const toggleBtn = completedSection.querySelector(
        ".dream-kpi-completed-toggle",
      );
      const completedGrid = completedSection.querySelector(
        ".dream-kpi-completed-grid",
      );
      toggleBtn.addEventListener("click", () => {
        ...
        toggleBtn.querySelector(".dream-kpi-completed-arrow").textContent =
          completedSectionCollapsed ? "▶" : "▼";
      });
  ...
  function renderKpiHistory() {
    ...
    `;
    headerRow
      .querySelector(".dream-kpi-history-log-btn")
      .addEventListener("click", () => showKpiLogModal(kpi));
    ...
    if (logs.length === 0) {
      ...
      logs.forEach((log) => {
        ...
        `;
        item
          .querySelector(".dream-kpi-history-edit")
          .addEventListener("click", () => showKpiLogModal(kpi, log));
        item
          .querySelector(".dream-kpi-history-delete")
          .addEventListener("click", () => {
    ...
    todos.forEach((todo) => {
      ...
      item
        .querySelector(".dream-kpi-todo-sub-add")
        .addEventListener("click", () => {
          ...
          <button type="button" class="dream-kpi-todo-del" title="삭제">×</button>
        `;
          const subInput = subItem.querySelector(
            ".dream-kpi-todo-subitem-input",
          );
          subInput.focus();
          subInput.addEventListener("blur", () => {
            ...
            if (val === "") {
              ...
              span.className =
                "dream-kpi-todo-text dream-kpi-todo-subitem-text";
          ...
          subItem
            .querySelector(".dream-kpi-todo-check")
          ...
          while (
            insertBefore &&
            insertBefore.classList.contains("dream-kpi-todo-subitem")
          ) {
      ...
      subtasks.forEach((st) => {
        const subItem = document.createElement("div");
        subItem.className =
          "dream-kpi-todo-subitem" + (st.done ? " is-completed" : "");
    ...
    modal.innerHTML = `
      ...
      <div class="dream-kpi-panel dream-path-context-panel">
        ...
        <form class="dream-kpi-form dream-path-edit-form">
          <div class="dream-kpi-field">
            <label>행복 이름</label>
            <input type="text" name="name" value="${escapeHtml(happiness.name || "")}" placeholder="행복 이름" />
          </div>
          <button type="submit" class="dream-kpi-submit">수정</button>
        </form>
