...
function saveHappinessMap(data) {
  try {
    localStorage.setItem(HAPPINESS_MAP_STORAGE_KEY, JSON.stringify(data));
  } catch (_) {}
}
...
export function render() {
  ...
  function showHappinessEditModal(happiness) {
    const modal = document.createElement("div");
    modal.className = "dream-kpi-modal";
    modal.innerHTML = `
      <div class="dream-kpi-backdrop"></div>
      <div class="dream-kpi-panel">
        <div class="dream-kpi-modal-header">
          <h3 class="dream-kpi-modal-title">행복 이름 수정</h3>
          <button type="button" class="dream-kpi-modal-close" title="닫기">×</button>
        </div>
        <form class="dream-kpi-form">
          <div class="dream-kpi-field">
  ...
  function renderTabs() {
    ...
    data.happinesses.forEach((happiness) => {
      ...
      tab.innerHTML = `
        <span class="dream-tab-text">${escapeHtml(happiness.name || "행복 이름")}</span>
        <button type="button" class="dream-tab-edit" title="행복 이름 수정">✎</button>
        <button type="button" class="dream-tab-del" title="행복 삭제 (내부 KPI·로그·할일 모두 삭제)">×</button>
      `;
      ...
      tab.querySelector(".dream-tab-edit").addEventListener("click", (e) => {
        e.stopPropagation();
        showHappinessEditModal(happiness);
      });
  ...
  addBtn.addEventListener("click", () => {
    const data = loadHappinessMap();
    const happiness = { id: nextId(), name: "새 행복" };
