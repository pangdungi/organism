 ...
 *
 * @param {object[]} kpis
 * @param {"all"|"active"|"completed"|string} kpiFilter
...
export function buildKpiListPaintSignature(kpis, kpiFilter, progressFor, ledgerCategory) {
  const completedKpis = kpis.filter((k) => progressFor(k).isCompleted);
  const activeKpis = kpis.filter((k) => !progressFor(k).isCompleted);
  const listToShow =
    kpiFilter === "active"
      ? activeKpis
      : kpiFilter === "completed"
