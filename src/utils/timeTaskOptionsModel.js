...
/** KPI 삭제 후 남은 같은 이름·kpiId 없는 과제 행 제거 (재추가 시 옛 id 재사용 방지) */
function filterStaleKpiOrphanTaskRows(rows) {
  const keepers = getActiveKpiTaskKeepersById();
  const activeKpiIds = new Set(keepers.keys());
  const activeKpiNames = new Set(
    [...keepers.values()].map((m) => m.name).filter(Boolean),
  );
...
/** KPI 맵의 현재 표시명·category 반영 + 진행전·완료 연동 과제 제거 */
export function patchKpiLinkedTasksFromKpiMaps() {
  ...
  try {
    const keepers = getActiveKpiTaskKeepersById();
    const allKpiIds = getAllKpiIdsFromMaps();
    let changed = false;
    const next = [];
    for (const o of _ledgerTasksMem) {
      const kid = String(o.kpiId || "").trim();
      /* 맵에 있는데 진행중이 아니면(진행전·완료) 과제목록에서 제외. 맵 미로드 시에는 건드리지 않음 */
      if (
        kid &&
        allKpiIds.size > 0 &&
        allKpiIds.has(kid) &&
...
function mirrorTaskOptionsToLocalStorage() {
  ...
  if (!uid || _ledgerTasksMem == null || !Array.isArray(_ledgerTasksMem))
    return;
  try {
    setScopedLocalStorageItem(
      TASK_OPTIONS_KEY,
      JSON.stringify(_ledgerTasksMem),
      uid,
    );
  } catch (_) {}
}
...
function scheduleKpiTaskServerIdRealign(kpiId) {
  ...
  _kpiTaskRealignTimers.set(
    ...
    setTimeout(() => {
      ...
      void import("./timeLedgerTasksSupabase.js")
        .then((m) => m.pullTimeLedgerTasksFromSupabase({ ignoreSkip: true }))
        .catch(() => {});
...
export function getTaskOptionByName(name) {
  ...
  const matches = getFullTaskOptions().filter(
    (o) => (o.name || "").trim() === n,
  );
  if (!matches.length) return null;
  if (matches.length === 1) return matches[0];
  const activeKpiIds = new Set(getActiveKpiTaskKeepersById().keys());
  const activeKpiLinked = matches.filter((o) => {
    const kid = String(o.kpiId || "").trim();
    return kid && activeKpiIds.has(kid);
  });
  if (activeKpiLinked.length === 1) return activeKpiLinked[0];
  return matches.find((o) => String(o.kpiId || "").trim()) || matches[0];
}
...
export function kpiTimeTaskAdd(kpi, category) {
  ...
  if (!isKpiEligibleForTimeTaskList(kpi)) return;
  if (
...
/** KPI 과제 행이 없으면 추가 — KPI id마다 항상 새 과제 uuid (이름 재사용 금지) */
...
export function kpiTimeTaskRemove(kpi, _syncNameFromMap) {
  const kpiId = (kpi && kpi.id && String(kpi.id).trim()) || "";
  ...
  kpiSyncDebugLog("KPI 과제 삭제", {
    kpiId,
    kpiName: (kpi && (kpi.name || "").trim()) || "",
    removedTaskId: removedId || "(없음)",
  });
...
export function kpiTimeTaskRename(kpi, oldNameFromKpi) {
  ...
  const idx = opts.findIndex((o) => String(o.kpiId || "").trim() === kpiId);
  if (idx < 0) return;
...
export function remapTimeLedgerEntryTaskIds(idPairs) {
  const pairs = (Array.isArray(idPairs) ? idPairs : [])
    ...
    .filter(
      (p) =>
        p.from && p.to && p.from !== p.to && isUuid(p.from) && isUuid(p.to),
    );
...
export function migrateTimeLogRowsTaskIds() {
  ...
  try {
    ...
    const next = arr.map((r) => {
      ...
      if (tid) {
        /* 삭제된 KPI 과제 id — 이름으로 새 KPI 과제에 재연결하지 않음 */
        return row;
      }
      ...
      if (kpiSyncedNames.has(n)) {
        const kpiLinked = opts.filter(
          (o) => (o.name || "").trim() === n && String(o.kpiId || "").trim(),
        );
      ...
      if (matches.length === 1 && isUuid(String(matches[0].id || ""))) {
        changed = true;
        return { ...row, taskId: String(matches[0].id).trim() };
      }
      return row;
    });
    if (changed) {
...
export function applyTimeLedgerTasksFromServer(
  ...
  for (const t of builtinTemplates) {
    ...
    for (const { from, to } of C.MEAL_TASK_NAME_RENAMES) {
      if (to === t.name) {
        builtInIdSet.add(deterministicTaskId(from, t.productivity, t.category));
      }
    }
  }
  ...
  for (const r of serverRowsSafe) {
    ...
    const kid = fromServerKpi || (loc && String(loc.kpiId || "").trim()) || "";
    /* 진행전·완료 KPI 연동 과제는 pull로도 목록에 넣지 않음 */
    if (kid && allKpiIds.size > 0 && allKpiIds.has(kid) && !keepers.has(kid)) {
      continue;
    }
    /* 이 사람 KPI 목록에 없는 id — 삭제한 행동을 서버 스냅샷으로 다시 넣지 않음 */
    if (kid && allKpiIds.size > 0 && !allKpiIds.has(kid)) {
      continue;
    }
    outIds.add(rid);
  ...
  const serverKpiIdSet = new Set(
    serverRowsSafe.map((r) => String(r.kpi_id ?? "").trim()).filter((k) => k),
  );
