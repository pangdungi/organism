-- ui_font_id: 그리운 코코최윤체·몽토리·체리1스푼 제거 → 온글잎 밑미(mitmi)

update public.user_subscriptions
set ui_font_id = 'mitmi'
where ui_font_id in ('cocochoitoon', 'mongtori', 'cherryspoon', 'ryuryu');

comment on column public.user_subscriptions.ui_font_id is
  '앱 UI 글꼴 id; mitmi | gongbujahana | myeoneunheulrim | leeseoyun (+ legacy ids)';
...
begin
  ...
  if v_id in ('ryuryu', 'cocochoitoon', 'mongtori', 'cherryspoon') then
    v_id := 'mitmi';
  end if;
  if v_id not in (
    'system',
