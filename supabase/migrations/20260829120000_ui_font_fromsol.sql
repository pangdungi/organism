-- 그리운 프롬솔(fromsol)을 UI 글꼴 목록에 다시 넣고, 신규 기본값으로 둔다.
-- 이미 ui_font_id가 있는 계정은 바꾸지 않는다.

alter table public.user_subscriptions
  alter column ui_font_id set default 'fromsol';

comment on column public.user_subscriptions.ui_font_id is
  '앱 UI 글꼴 id; fromsol | mitmi | gongbujahana | myeoneunheulrim | leeseoyun | uhbeezziba | oksooni | myoeunddobak (+ legacy ids)';
...
begin
  ...
  if v_id in ('ryuryu', 'cocochoitoon', 'mongtori', 'cherryspoon', 'uhbeerice') then
    v_id := 'mitmi';
  end if;
  if v_id not in (
    'system',
